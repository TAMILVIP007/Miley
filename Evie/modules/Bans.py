...
async def dban(event):
    ...
    if sender_id != OWNER_ID:
        ...
        if not await can_ban_users(message=event):
            ...
            return
    if user and user.id in [BOT_ID, OWNER_ID]:
...
async def dban(event):
    ...
    await event.reply('Yep, this user can join!')
